...
import { View, Text, StyleSheet, ScrollView, StatusBar, TouchableOpacity, Linking, Image } from 'react-native';
import { useNavigation } from '@react-navigation/native';
import Ionicons from 'react-native-vector-icons/Ionicons';
import { SafeAreaView } from 'react-native-safe-area-context';
    ...
    <SafeAreaView style={{ flex: 1 }}>
      ...
      <View style={styles.container}>
        ...
        <ScrollView contentContainerStyle={styles.scrollContent}>
          ...
          <View style={styles.pdfRow}>
            {pdfs.map((item, idx) => (
              <View key={item._id || idx} style={styles.pdfCard}>
                <Image
                ...
                />
                <Text style={styles.pdfTitle}>{item.title}</Text>
                <TouchableOpacity style={styles.downloadButton} onPress={() => openPdf(item.pdfUrl)}>
                  <Ionicons name="download" size={16} color="#fff" />
                  <Text style={styles.downloadText}>Download</Text>
                </TouchableOpacity>
              </View>
